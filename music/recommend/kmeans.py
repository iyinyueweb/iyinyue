...
def kmeans(data_set, k, dist_measure=dist_eclud, create_center=random_center):
    ...
    while cluster_changed:
        ...
        # 更新族心位置
        for cent in range(k):
            cluster = data_set[np.nonzero(cluster_ss[:, 0].A == cent)[0]]  # 取出同一族的向量集合
            centroids[cent, :] = np.mean(cluster, axis=0)  # 计算族中心
    return centroids, cluster_ss


# 二分kmeans算法
# 克服了kmeans算法收敛于局部最小值的问题
# 基本思路：
#   将所有点都看成一个簇
#   当簇数< K时：
#       对每一个簇
#           计算总误差
#           在给定的簇上在进行kmeans聚类（K=2)
#           计算将该簇一分为二的总误差
#       选择使得误差最小的那个簇进行划分操作
def binary_kmeans(data_set, k, dist_measure=dist_eclud):
    ...
    for i in range(m):
        cluster_ss[i, 1] = dist_measure(np.mat(centroid0), data_set[i, :]) ** 2  # 计算各点距簇中心的距离
    while len(center_list) < k:
        lowest_sse = np.inf  # 最小误差平方和
        for i in range(len(center_list)):
            # 获取属于当前簇的向量
            current_cluster_index = data_set[np.nonzero(cluster_ss[:, 0].A == i)[0], :]
            centroid_mat, split_cluster_ss = kmeans(current_cluster_index, 2, dist_measure)  # 二分聚类
            sse_of_split = np.sum(split_cluster_ss[:, 1])  # 获取当前划分簇的误差平方和
            # 计算为被划分的误差
            sse_of_not_split = np.sum(cluster_ss[np.nonzero(cluster_ss[:, 0].A != i)[0], 1])
            if sse_of_split + sse_of_not_split < lowest_sse:  # 若一分为二的总误差小于最小误差
        ...
        best_cluster_ss[np.nonzero(best_cluster_ss[:, 0].A == 1)[0], 0] = len(center_list)  # 更新所属簇
        best_cluster_ss[np.nonzero(best_cluster_ss[:, 0].A == 0)[0], 0] = best_center_to_split_index  # 更新所属簇
        center_list[best_center_to_split_index] = best_new_centroids[0, :].tolist()[0]  # 用第一个划分替换当前的划分点
